...
public abstract class PageLoader {
     ...
     * @return
     */
    public List<ChapterBean> getChapterCategory() {
        return mChapterList;
    }
    ...
    public void saveRecord() {
        ...
        mCollBook.setHistoryBookNum(mCurChapterPos);

        if (mCurPage != null) {
            mCollBook.setLastReadPosition(mCurPage.position);
        } else {
            mCollBook.setLastReadPosition(0);
        }
        //存储到数据库
    ...
    private void prepareBook() {
        mCurChapterPos = mCollBook.getHistoryBookNum();
    ...
    private List<TxtPage> loadPageList(int chapterPos) throws Exception {
        // 获取章节
        ChapterBean chapter = mChapterList.get(chapterPos);
        // 判断章节是否存在
     ...
     * 刷新章节列表
     */
    public abstract void refreshChapterList() throws IOException;
    ...
    private void drawBackground(Bitmap bitmap, boolean isUpdate) {
        ...
        if (!isUpdate) {
            ...
            if (!mChapterList.isEmpty()) {
                ...
                if (mStatus != STATUS_FINISH) {
                    if (isChapterListPrepare) {
                        canvas.drawText(mChapterList.get(mCurChapterPos).getChapterTitle(), mMarginWidth, tipTop- ScreenUtils.dpToPx(5), mTipPaint);
                    }
                } else {
                    canvas.drawText(mCurPage.title, mMarginWidth, tipTop+ ScreenUtils.dpToPx(5), mTipPaint);
                }
                ...
                if (mStatus == STATUS_FINISH) {
                    ...
                    canvas.drawText(percent, mMarginWidth, y- ScreenUtils.dpToPx(5), mTipPaint);
                }
            }
        } else {
            //擦除区域
        ...
        Rect polar = new Rect(polarLeft, polarTop- ScreenUtils.dpToPx(5), visibleRight, polarTop + polarHeight - ScreenUtils.dpToPx(7));
        ...
        Rect outFrame = new Rect(outFrameLeft, outFrameTop- ScreenUtils.dpToPx(5), polarLeft, outFrameBottom- ScreenUtils.dpToPx(5));
        ...
        RectF innerFrame = new RectF(outFrameLeft + border + innerMargin, outFrameTop + border + innerMargin- ScreenUtils.dpToPx(5), outFrameLeft + border + innerMargin + innerWidth, outFrameBottom - border - innerMargin- ScreenUtils.dpToPx(5));
        ...
        String time = Utils.dateConvert(String.valueOf(System.currentTimeMillis()), "HH:mm");
    ...
    private void drawContent(Bitmap bitmap) {
        ...
        if (mStatus != STATUS_FINISH) {
            ...
            for (int i = mCurPage.titleLines; i < mCurPage.lines.size(); ++i) {
                String[] title1s = mCurPage.lines.get(i).split("章");
                String[] title2s = new String[2];
                if (str != null) {
                    title2s = str.split("章");
                }
                if (title1s.length >= 2 && title2s.length >= 2) {
                    if (!title1s[1].trim().equals(title2s[1].trim())) {
                        str = mCurPage.lines.get(i);
                        boolean hasLineFeed = str.contains("\n");
    ...
    private void preLoadNextChapter() {
        ...
        //调用异步进行预加载加载
        Single.create((SingleOnSubscribe<List<TxtPage>>) e -> e.onSuccess(loadPageList(nextChapter))).compose(RxUtils::toSimpleSingle).subscribe(new SingleObserver<List<TxtPage>>() {
            @Override
     ...
     * @return
     */
    private List<TxtPage> loadPages(ChapterBean chapter, BufferedReader br) {
        //生成的页面
        ...
        try {
            while (showTitle || (paragraph = br.readLine()) != null) {
                ...
                if (!showTitle) {
                    ...
                    paragraph = Utils.halfToFull("  " + paragraph + "\n");
                } else {
                    //设置 title 的顶部间距
                    rHeight -= mTitlePara;
                }
    ...
    /**
     * @return:获取初始显示的页面 此处有bug
     */
    private TxtPage getCurPage(int pos) {
        if (mPageChangeListener != null) {
            mPageChangeListener.onPageChange(pos);
        }
        if (pos < mCurPageList.size()) return mCurPageList.get(pos);
        else return mCurPageList.get(mCurPageList.size() - 1);
    }
    ...
    public interface OnPageChangeListener {
         ...
         */
        void requestChapters(List<ChapterBean> requestChapters);
